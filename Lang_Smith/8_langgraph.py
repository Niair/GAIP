...
def evaluate_language(state: UPSCState) -> UPSCState:
    prompt = (
        f"""Evaluate the language quality of the following essay and provide feedback
        and assign a score out of 10.\n\n {state['essay']}"""
    )
    out = structured_model.invoke(prompt)
    return {"language_feedback": out.feedback, "individual_scores": [out.scores]}

@traceable(name="evaluate_analysis", tags=["dimension:analysis"], metadata={"dimension": "analysis"})
def evaluate_analysis(state: UPSCState) -> UPSCState:
    prompt = (
        f""" Evaluate the depth of analysis of the following essay and provide feedback
        and assign a score out of 10.\n\n {state['essay']} """
    )
    out = structured_model.invoke(prompt)
    return {"analysis_feedback": out.feedback, "individual_scores": [out.scores]}
...
def evaluate_thought(state: UPSCState) -> UPSCState:
    prompt = (
        f""" Evaluate the clarity of thought of the following essay and provide feedback
        and assign a score out of 10.\n\n {state['essay']} """
    )
    out = structured_model.invoke(prompt)
    return {"clarity_feedback": out.feedback, "individual_scores": [out.scores]}
